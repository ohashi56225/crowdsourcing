function create_table(table_id, dialog){
    var t_re = "";
    let table_title = table_id + "つ目の会話";
    ...
    for (var i = 0; i < dialog_turns.length; i++) {
        var table_tmp = '<tr>'
        + '<td style = "text-align:center">' + dialog_turns[i][1] + '</td>'
        if (dialog_turns[i][1] == "system") {
            ...
            if (dialog_turns[i][2] != "") {
                table_tmp += '<td>'
                + '<p style="background-color:f5f5f5;margin:2 0px;display:inline-block;">' + dialog_turns[i][2] +'</p>'
...
function submit(table_id) {
    ...
    if(document.getElementById("WorkerID").value==""||age==""||gender==""){
        all_checked = false;
        alert("Please fill in your worker ID, age and gender."); 
        return;
    }
    ...
    else alert("Please check all the contents."); 
};